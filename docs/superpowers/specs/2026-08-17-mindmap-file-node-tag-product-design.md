...
- 复制按钮具有明确可访问名称，分享地址输入框具备标签。分享分为匿名只读链接和编辑邀请链接：编辑邀请公开读取只能返回文件名与登录要求，不能返回脑图正文；领取必须经过登录、全局编辑权限和链接生命周期校验，再在文件主锁内新增或提升当前用户的协作者权限。禁用编辑邀请只阻止后续领取，不隐式移除已经加入的协作者，界面必须提前说明这一边界。
...
## 142. 同节点并发条件修复与长期会话写栅栏设计

结构化节点补丁只能修复旧客户端独立创建同名嵌套 Y.Map 造成的兼容问题，不能成为覆盖 Yjs 冲突决议的第二套同步协议。更新补丁必须同时携带操作前后的节点数据；接收端逐字段执行 compare-and-set：当前值已经等于目标值时不写，仍等于前态时才修复，出现第三个值说明存在并发编辑，必须保留 Yjs 已选出的确定性结果。子节点顺序使用相同的前态条件替换。没有前态的旧补丁只允许补齐缺失字段，不能覆盖现有字段。顶层节点删除完全由 Yjs update 决定；节点账本已经证明某 UID 曾存在但当前缺失时，任何更新快照都不得复活它，只有旧状态从未包含该节点时才允许执行缺失节点兼容修复。

补丁能力拆分为基础 `structured-node-patch-v1` 与明确承诺上述条件语义的 `conditional-node-patch-v1`。服务端只向声明条件能力的连接发送结构化补丁并省略冗余完整状态；旧客户端收到标准 Yjs 增量及可用的完整状态，绝不能继续获得会无条件覆盖整节点的修复快照。检查点抑制仍要求结构化补丁和检查点能力同时存在。能力值由服务端白名单协商，不能因为客户端发送任意字符串而进入新协议分支。

WebSocket 首次认证只证明连接建立时可访问，不能授权整个长会话。update、兼容 sync_step、checkpoint 和重新竞争 seed 的 request_seed 都必须在处理时重新通过数据库编辑权限与正文 revision 栅栏；明确撤权、归档或删除立即终止连接并禁止断开检查点回写。数据库或权限服务暂时不可用时同样先停止广播和回写，但返回可重试认证错误，以 1013 关闭当前传输；客户端保留 Y.Doc、未保存修改和草稿，沿有界退避重新认证。验收至少覆盖同节点不同字段、同字段竞争、更新/删除竞争、旧补丁降级、能力裁剪、只读种子拒绝、旧 revision 种子拒绝、临时鉴权故障无泄漏自动重连、明确撤权永久终止，以及完整前后端回归和生产构建。

## 143. 同账号多浏览器 Presence 与节点短租约设计

内容归属身份与 Presence 身份必须分离：正文权限、审计和保存继续使用账号 `userId`，光标、选区和节点占用必须使用服务端在每条 WebSocket 连接认证时生成的不可伪造 `sessionId`。客户端只能忽略与自身 `sessionId` 完全相同的 awareness；同一账号的其他浏览器仍是独立在线会话，应显示“其他浏览器”标识并拥有独立选区。节点协作者列表同样按 `sessionId` 去重和删除，单个标签页退出只能清理自己的标记，不能按账号误删仍在线浏览器的节点占用。滚动升级期间，无 `sessionId` 的旧服务端消息继续按 `userId` 兼容，但同账号旧消息只允许忽略，不能猜测出不存在的会话身份。

Awareness 是易失状态，不进入数据库或脑图历史。可编辑会话认证后立即发布当前选区，每 15 秒刷新一次；远端会话 30 秒未刷新即过期。收到权威 `room_users` 后，已有连接再次发布选区，使新加入的浏览器无需等待下一次点击即可看到占用。节点选区会被编辑端解释为短租约，因此客户端不允许只读会话发送，服务端也必须按认证后的编辑权限把只读消息收敛为空选区，防止查看者阻塞编辑。正常退出和连接最终清理都广播携带原 `sessionId` 的空选区；网络异常、进程崩溃或 Pub/Sub 丢包由超时回收兜底。定时器在断线、销毁和会话终止时清理，暂停历史预览时不发送正式文档选区。节点文本当前仍是原子协作字段，因此编辑器启用“远端会话已占用时不进入编辑”的短租约式保护；该保护只降低同时编辑概率，不是数据正确性的锁，服务端 revision 栅栏、Yjs 合并和云端回源仍是最终一致性边界。

本阶段复用既有连接 UUID，不新增数据库对象或迁移。验收至少覆盖同账号两个及以上浏览器、当前会话自消息忽略、其他会话独立显示、单会话清除不误删、异常断线过期、加入房间后的选区补发、定时器销毁、旧服务端兼容、节点单击/双击/键盘激活占用门禁，以及完整前后端回归和生产构建。

## 144. 协作事件耐久化与单一事实源演进边界

当前 Redis Pub/Sub 只负责低延迟分发，不提供断线重放；数据库正文与 `content_revision` 仍是权威事实源，心跳发现 revision 超前后必须销毁旧 Y.Doc 并重新加载云端完整正文，不能只推进版本号或把离线旧状态合回新基线。跨 worker 广播必须让本地发送与 Redis 发布并行，单个慢 WebSocket 最多拖慢自身并触发安全退休，不能阻塞其他实例先收到更新。检查点仍按 revision 隔离和来源集合压缩，任何未持久化实时消息都不能被宣称为已经可靠保存。

当协作规模、离线时长或审计要求超过 Pub/Sub + 周期检查点能力时，再以独立版本演进到耐久更新日志：先写 Redis Streams 或等价追加日志成功后再确认消息，消费者按文档与流序号幂等重放，Yjs state vector 计算差异更新，后台定期合并检查点并在确认所有消费者越过安全水位后裁剪旧日志。届时应把 Yjs 更新流作为实时正文的单一写入来源，SQL `node_tree` 只作为异步派生的查询/导出快照，避免 HTTP 保存与 Yjs 检查点形成两套互相覆盖的事实源。该演进需要灰度协议、容量与故障演练，不与本轮 Presence 修复混合发布。

## 145. 登录态编辑邀请与分享权限闭环设计

分享弹窗必须直接提供“仅查看（无需登录）”和“可编辑（需要登录）”两种用户可理解的选择，不能把编辑能力隐藏在另一个侧栏后再让用户猜测。两类链接沿用相同的随机令牌、有效期、禁用和状态展示；列表明确标识权限。可编辑链接是加入协作的邀请凭据，不是匿名写入凭据：未登录访问时仅展示邀请说明和登录入口，公开接口不得返回节点树、主题、视图或文档配置；登录后由受保护接口领取。

领取流程先校验令牌格式，再以脑图主记录、分享记录、协作者记录的固定锁顺序重新确认链接有效、未过期、类型为编辑、文件未归档且当前账号有效。所有者直接进入自己的文件；既有只读协作者提升为编辑，既有编辑协作者幂等返回，尚未加入的用户新增为编辑协作者。领取后的保存、WebSocket、评论和列表访问继续复用现有资源权限模型，不新增绕过身份的写路径。禁用链接与移除成员是两个明确动作：禁用只阻止后续领取，已加入成员仍由协作者管理调整或移除。

分享页必须保留路由令牌完成登录回跳，领取请求支持取消并拒绝文件切换后的迟到响应；成功返回有效文件 ID 后进入共享编辑器，失败留在邀请页显示可重试的业务原因。验收至少覆盖只读链接仍可匿名预览、编辑链接不泄露正文、未登录回跳、首次领取、只读升级、重复领取、所有者访问、过期/禁用/错误类型/归档拒绝、全局权限门禁、响应式邀请页、完整前后端回归和生产构建。本阶段复用 `mindmap_share.share_type` 与协作者表，不新增数据库迁移。
