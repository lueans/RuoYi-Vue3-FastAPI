...
class RichText {
  ...
  updateTextEditNode() {
    ...
    const textAutoWrapWidth = this.node.hasCustomWidth()
      ? this.node.customTextWidth
      : this.mindMap.opt.textAutoWrapWidth
    this.addNodeTextStyleToTextEditNode(this.node)
    this.textEditNode.style.marginLeft = `-${paddingX * scaleX}px`
    this.textEditNode.style.marginTop = `-${paddingY * scaleY}px`
  ...
  handleDataToRichText(data) {
    ...
    if (!oldIsRichText && !isUndef(data.text)) {
      data.text = htmlEscape(String(data.text))
    }
  }
