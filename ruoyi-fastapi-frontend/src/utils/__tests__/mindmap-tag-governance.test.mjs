...
test('标签管理支持分组、影响评估、替换和有界批量归档', async () => {
  ...
  assert.match(source, /:title="categoryForm\.id \? '编辑标签分组' : '新建标签分组'"/)
  ...
  assert.match(source, /handle="\.tagGroupDragHandle"/)
  assert.match(source, /finishCategoryReorder\('global'\)/)
  assert.match(source, /finishCategoryReorder\('mine'\)/)
  assert.match(source, /全局分组/)
  assert.match(source, /我的分组/)
  assert.match(source, /v-model="categoryForm\.showOnHome"/)
  assert.match(source, /v-model="categoryForm\.selectionMode"/)
  assert.match(source, /标签首页/)
  assert.match(source, /仅更多标签/)
  assert.match(source, />单选</)
  assert.match(source, />多选</)
  assert.match(source, /updateTagCategory\([\s\S]*categoryForm\.selectionMode/)
  assert.match(source, /addTagCategory\([\s\S]*categoryForm\.selectionMode/)
  assert.match(source, /reorderTagCategories\(categoryIds\)/)
  assert.match(source, /Number\(category\.tagCount\) > 0/)
