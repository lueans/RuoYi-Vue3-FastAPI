...
test('会话终止事件贯穿 Yjs、编辑器草稿保护和页面反馈', async () => {
  ...
  assert.match(editorSource, /entry\.fallbackSaved = saveMindmapDraftFallbackSync\(entry\.options\)/)
  assert.match(editorSource, /downloadConflictBackup\(entry\.document, entry\.downloadPrefix\)/)
